...
export default function CategorySectionDynamic({ collections }: CategorySectionDynamicProps) {
  ...
  const extendedCategories = [...validCollections, ...validCollections, ...validCollections];
            ...
            <div className="relative">
              ...
              <Slider ref={sliderRef} {...settings}>
                {extendedCategories.map((category, index) => (
                  <div key={`${category.handle}-${index}`} className="px-3">
                    <Link href={category.path} className="group relative block">
