...
interface TrabajosCarruselProps {
  trabajos: Trabajo[];
}

export function TrabajosCarrusel({ trabajos }: TrabajosCarruselProps) {
  ...
  const totalPages = Math.ceil(trabajos.length / visibleCards);
  ...
  const handleScroll = useCallback(() => {
    ...
    // Normalizar el índice para que siempre esté entre 0 y trabajos.length - 1
    const normalizedIndex = closestIndex % trabajos.length;
    setActiveIndex(normalizedIndex);

    // Si estamos en la segunda mitad duplicada, saltar sin animación al inicio
    if (closestIndex >= trabajos.length && !container.dataset.isLooping) {
  ...
  const scrollToPage = (pageIndex: number) => {
    ...
    scrollToCard(Math.min(cardIndex, trabajos.length - 1));
  };
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-6 sm:gap-8 lg:gap-12 items-center">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            />
            <p className="text-white/70 text-sm text-center lg:text-left hidden lg:block">
              Explora nuestros trabajos realizados para empresas líderes
            </p>
          </div>
          ...
          <div className="lg:col-span-8">
            <div className="relative">
              ...
              >
                ...
                {[...trabajos, ...trabajos].map((trabajo, index) => {
                  return (
                    <div
                      key={`${trabajo.empresa}-${index}`}
                    ...
                    >
                      <div className="group relative block aspect-[3/4] w-full overflow-hidden bg-gray-200 rounded-sm">
                        <Image
                          src={trabajo.imagen}
                          alt={`Trabajo para ${trabajo.empresa}`}
                          fill
